...
lib_t LangLua::GetInterface() { return reinterpret_cast<lib_t>(lua); }

LangLua::LangLua(lua_State *lua) { this->lua = lua; }
...
struct LuaFunctionDispatcher {
  ...
  inline static ReturnType Dispatch(lua_State *&&lua, Args &&...args) noexcept {
    ...
    constexpr ScriptFunctionData const &functionData =
        ScriptFunctions::functions[FunctionIndex];
    // Retrieve argument from the Lua stack
    auto argument = luabridge::Stack<typename CharType<
        functionData.func.types[ArgIndex - 1]>::type>::get(lua, ArgIndex);
    // Recursively dispatch the Lua function
    return LuaFunctionDispatcher<ArgIndex - 1, FunctionIndex>::
        template Dispatch<ReturnType>(std::forward<lua_State *>(lua), argument,
...
struct LuaFunctionDispatcher<0, FunctionIndex> {
  ...
  inline static ReturnType Dispatch(lua_State *&&, Args &&...args) noexcept {
    ...
    return reinterpret_cast<FunctionEllipsis<ReturnType>>(
        functionData.func.addr)(std::forward<Args>(args)...);
  }
};
...
static typename std::enable_if<
    ScriptFunctions::functions[FunctionIndex].func.ret == 'v', int>::type
LuaFunctionWrapper(lua_State *lua) noexcept {
  // Dispatch the Lua function
  LuaFunctionDispatcher<
      ScriptFunctions::functions[FunctionIndex].func.numargs,
...
LuaFunctionWrapper(lua_State *lua) noexcept {
  // Dispatch the Lua function
  auto result = LuaFunctionDispatcher<
      ScriptFunctions::functions[FunctionIndex].func.numargs, FunctionIndex>::
      template Dispatch<typename CharType<
          ScriptFunctions::functions[FunctionIndex].func.ret>::type>(
          std::forward<lua_State *>(lua));
  // Push the result onto the Lua stack
  luabridge::Stack<typename CharType<ScriptFunctions::functions[FunctionIndex]
                                         .func.ret>::type>::push(lua, result);
  return 1;
}

// Struct for defining Lua functions with names and wrappers
template <unsigned int FunctionIndex> struct LuaFunctionDefinition {
  static constexpr LuaFunctionData FunctionInfo{
      ScriptFunctions::functions[FunctionIndex].name,
      LuaFunctionWrapper<FunctionIndex>};
};

template <> struct LuaFunctionDefinition<0> {
  static constexpr LuaFunctionData FunctionInfo{"CreateTimer",
                                                LangLua::CreateTimer};
};
template <> struct LuaFunctionDefinition<1> {
  static constexpr LuaFunctionData FunctionInfo{"CreateTimerEx",
                                                LangLua::CreateTimerEx};
};
template <> struct LuaFunctionDefinition<2> {
  static constexpr LuaFunctionData FunctionInfo{"MakePublic",
                                                LangLua::MakePublic};
};
template <> struct LuaFunctionDefinition<3> {
  static constexpr LuaFunctionData FunctionInfo{"CallPublic",
                                                LangLua::CallPublic};
};
...
template <size_t... Indices> LuaFuctionData *functions(indices<Indices...>) {
  ...
  static_assert(sizeof(functions_) / sizeof(functions_[0]) ==
                    sizeof(ScriptFunctions::functions) /
                        sizeof(ScriptFunctions::functions[0]),
                "Not all functions have been mapped to Lua");

  return functions_;
}
#else
template <unsigned int I> struct LuaFunctionInitializer {
...
template <> struct LuaFunctionInitializer<0> {
  constexpr static void Initialize(LuaFunctionData *functions_) {
    functions_[0] = LuaFunctionDefinition<0>::FunctionInfo;
  }
};

template <size_t LastI> LuaFunctionData *GetLuaFunctions() {
...
void LangLua::LoadProgram(const char *filename) {
  ...
  if ((err = luaL_loadfile(lua, filename)) != 0)
    throw std::runtime_error("Lua script " + std::string(filename) +
                             " error (" + std::to_string(err) + "): \"" +
                             std::string(lua_tostring(lua, -1)) + "\"");

  constexpr auto functions_n = sizeof(ScriptFunctions::functions) /
                               sizeof(ScriptFunctions::functions[0]);

#ifdef __arm__
  LuaFunctionData *functions_ = GetLuaFunctions(IndicesFor<functions_n>{});
#else
  LuaFunctionData *functions_ =
      GetLuaFunctions<sizeof(ScriptFunctions::functions) /
                      sizeof(ScriptFunctions::functions[0])>();
#endif
  luabridge::Namespace dreamweave =
      luabridge::getGlobalNamespace(lua).beginNamespace("dreamweave");
  ...
  if ((err = lua_pcall(lua, 0, 0, 0)) !=
      0) // Run once script for load in memory.
    throw std::runtime_error("Lua script " + std::string(filename) +
                             " error (" + std::to_string(err) + "): \"" +
                             std::string(lua_tostring(lua, -1)) + "\"");
}
...
boost::any LangLua::Call(const char *name, const char *argl, int buf, ...) {
  ...
  for (int index = 0; index < n_args; index++) {
    ...
    case 'l':
      luabridge::Stack<unsigned long long>::push(
          lua, va_arg(vargs, unsigned long long));
      break;

    case 'w':
      luabridge::Stack<signed long long>::push(lua,
                                               va_arg(vargs, signed long long));
      break;
    ...
    default:
      throw std::runtime_error("C++ call: Unknown argument identifier " +
                               argl[index]);
    }
  }
...
boost::any LangLua::Call(const char *name, const char *argl,
  ...
  for (int index = 0; index < n_args; index++) {
    switch (argl[index]) {
    case 'i':
      luabridge::Stack<unsigned int>::push(
          lua, boost::any_cast<unsigned int>(args.at(index)));
      break;

    case 'q':
      luabridge::Stack<signed int>::push(
          lua, boost::any_cast<signed int>(args.at(index)));
      break;

    case 'l':
      luabridge::Stack<unsigned long long>::push(
          lua, boost::any_cast<unsigned long long>(args.at(index)));
      break;

    case 'w':
      luabridge::Stack<signed long long>::push(
          lua, boost::any_cast<signed long long>(args.at(index)));
      break;

    case 'f':
      luabridge::Stack<double>::push(lua,
                                     boost::any_cast<double>(args.at(index)));
      break;

    case 'p':
      luabridge::Stack<void *>::push(lua,
                                     boost::any_cast<void *>(args.at(index)));
      break;

    case 's':
      luabridge::Stack<const char *>::push(
          lua, boost::any_cast<const char *>(args.at(index)));
      break;
    ...
    case 'b':
      ...
      break;
    default:
      throw std::runtime_error("Lua call: Unknown argument identifier " +
                               argl[index]);
    }
  }
